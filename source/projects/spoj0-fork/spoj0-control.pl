...
elsif($cmd eq 'plag-export'){
	my $problem_id = shift @ARGV or Usage;
	&PlagExport($problem_id);
}
else{
	Usage;
}
...
sub ImportSet{
	...
	foreach my $pl(@pls){
		...
		if(!($pl =~ /^\..*/) && -d "$SETS_DIR/$set_code/$pl/"){
			...
			my $prob_dir = "$SETS_DIR/$set_code/$pl";
...
sub PlagExport{
	...
	my $run_st = $dbh->prepare(qq( 
		SELECT * FROM runs WHERE status='ok' AND problem_id=?
		ORDER BY run_id ASC
	));
	$run_st->execute($problem_id) or die "Unable to execute statment!";
	my $dir = "$EXPORT_DIR/plag/problem-$problem_id";
	mkdir($dir);
	my %passed = ();
	my $run;
	while($run = $run_st->fetchrow_hashref){
		...
		my $code = $$run{'source_code'};
		next if($passed{$user_id});
		my $ext = $$run{'language'};
		$passed{$user_id} = 1;
